...
int main()
{
    ...
    for (int i = 1; i <= checkoutLines; i++) {
        ...
        allCheckoutLines.push_back(tempName);//trying to add the queues to one big vector so I can access them?
    }
    ...
    while (minutes >= 0) {
        if (minutes < 720) {
            ...
            while (i < customers) {
                ...
                //cust1
                //cust.cartId = customerCount + 1; //oringial code commented out just in case
                ...
                while (linkedcount <= totalCustomersInStore.listCount()) {
                    ...
                    if (tempInfo.enterQTime == minutes) { //checking if customer needs to be moved to checkout line
                        while (k < allCheckoutLines.size()) {
                            queueNodeData tempQueueInfo, smallestLine; //holding the current element of the queue's data and holding the data of the smallest checkout line (unsure how to code)
                            ...
                            allCheckoutLines[smallestLineIndex].enQueue(/*need to add the current customer, but its linkedlist type ?*/);
